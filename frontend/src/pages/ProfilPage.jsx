...
export default function ProfilPage() {
  ...
  const handleWrapperPointer = (event) => {
    const wrapperEl = profilWrapperRef.current;
    if (!wrapperEl) return;

    const path = event.nativeEvent?.composedPath?.() || [];
    const touchesHeader = path.some(
      (el) => el instanceof HTMLElement && el.classList?.contains("fb-header")
    );
    ...
    const isProfileSurface = event.target.closest(".profil-wrapper, .profil-hero, .profil-content");
